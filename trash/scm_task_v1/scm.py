...
class WeightedScalarLayerConnection:
    ...
    2. assign normalized signed parent weights;
    3. sample one child mechanism:
       - edgewise scalar transforms followed by weighted aggregation;
       - weighted aggregation followed by a scalar transform;
       - joint MLP over weighted parent inputs.
    """
    ...
    def __init__(
        ...
        self.child_interactions = [None for _ in range(self.out_width)]
        ...
        for child in range(self.out_width):
            ...
            if method == self.EDGEWISE:
                for parent in parents.tolist():
                    self.edges[parent][child] = ScalarLatentEdge(
            ...
            elif method == self.POST_AGGREGATE:
                self.child_scalar_edges[child] = ScalarLatentEdge(
                    generator=self.g_dag,
                    device=device,
                    edge_family_probs=edge_family_probs,
                    small_mlp_hidden_dim=small_mlp_hidden_dim,
            ...
            elif method == self.INTERACTION:
                if parents.numel() < 2:
                    self.child_methods[child] = self.POST_AGGREGATE
                    self.child_scalar_edges[child] = ScalarLatentEdge(
                        generator=self.g_dag,
                        device=device,
                        edge_family_probs=edge_family_probs,
                    ...
                    continue
                pair_order = torch.randperm(parents.numel(), generator=self.g_dag, device=device)
                parent1 = int(parents[pair_order[0]].item())
                parent2 = int(parents[pair_order[1]].item())
                interaction_type = int(randint(0, 5, (), self.g_dag, device).item())
                self.child_interactions[child] = {
                    "parent1": parent1,
                    "parent2": parent2,
                    "interaction_type": interaction_type,
                }
            else:
                raise RuntimeError(f"Unknown child method: {method}")



    def _mlp_activation(self, x, activation_id):
    ...
    def _interaction(self, x1, x2, interaction_type):

        if interaction_type == 0:
            return x1 * x2
        if interaction_type == 1:
            return x1 * torch.sin(x2)
        ...
        for child in range(self.out_width):
            ...
            if parents.numel() == 0:
                ...
                if method == self.EDGEWISE:
                    ...
                    parameters = self.child_interactions[child]
                    if parameters is None:
                        raise RuntimeError(f"Missing interaction parameters for child={child}.")
                    parent1 = parameters["parent1"]
                    parent2 = parameters["parent2"]
                    x1 = self.weights[parent1, child] * parent_latents[parent1]
                    x2 = self.weights[parent2, child] * parent_latents[parent2]
                    value = self._interaction(x1, x2, parameters["interaction_type"])
...
class WeightedLayeredScalarSCM:
    ...
    def __init__(
        ...
        child_method_probs=(0.3, 0.2, 0.3, 0.2),
        source_prior_probs=(0.45, 0.20, 0.15, 0.05),
        joint_mlp_hidden_dim = 8,
        edge_family_probs=(0.50, 0.25, 0.25),
        ...
        self.child_method_probs = child_method_probs
        ...
        for layer in range(self.num_layers - 1):
            connection = WeightedScalarLayerConnection(
                ...
                g_x=self.g_x,
                device=self.device,
                child_method_probs=self.child_method_probs,
                source_prior_probs=self.source_prior_probs,
                joint_mlp_hidden_dim=joint_mlp_hidden_dim,
                edge_family_probs=edge_family_probs,
                small_mlp_hidden_dim=small_mlp_hidden_dim ,
                soft_tree_depth=soft_tree_depth,
                soft_tree_temperature=soft_tree_temperature,
