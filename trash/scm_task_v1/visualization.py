...
def plot_scm_graph(
    ...
    for layer_idx, connection in enumerate(scm.connections):
        ...
        for parent_idx in range(connection.in_width):
            for child_idx in range(connection.out_width):
                ...
                weight = float(weights[parent_idx, child_idx].item())
    ...
    nx.draw_networkx_edges(graph, positions, ax=ax, arrows=True, arrowstyle="-|>", arrowsize=15, width=1.0, alpha=0.45, node_size=900)
    # ordinary latent nodes
    nx.draw_networkx_nodes(graph, positions, nodelist=ordinary_nodes, node_size=900, node_shape="o", node_color="white", edgecolors="black", linewidths=1.3, ax=ax)
    # selected continuous features
    nx.draw_networkx_nodes(graph, positions, nodelist=continuous_nodes, node_size=1050, node_shape="o", node_color="0.55", edgecolors="black", linewidths=1.5, ax=ax)
    # selected categorical features
    categorical_collection = nx.draw_networkx_nodes(graph, positions, nodelist=categorical_nodes, node_size=1050, node_shape="o", node_color="white", edgecolors="black", linewidths=1.5, ax=ax)
    if categorical_collection is not None:
        categorical_collection.set_hatch("///")
    # target
    nx.draw_networkx_nodes(graph, positions, nodelist=[target_id], node_size=1700, node_shape="*", node_color="0.75", edgecolors="black", linewidths=1.5, ax=ax)
    ...
    nx.draw_networkx_labels(graph, positions, labels=node_labels, font_size=8, font_weight="bold", ax=ax)

    # Edge labels
    if show_edge_weights:
        nx.draw_networkx_edge_labels(graph, positions, edge_labels=edge_labels, font_size=6, rotate=False, label_pos=0.5, ax=ax)
    ...
    for layer_idx, width in enumerate(scm.widths):
        ...
        ax.text(layer_idx * x_gap, top_y, layer_name, ha="center", va="bottom", fontsize=12, fontweight="bold")
    ...
    for gid in feature_ids:
        ...
        x, y = positions[gid]
        ax.text(x, y - 0.75, f"I={importance:.3f}", ha="center", va="top", fontsize=7)
    ...
    ax.set_title("Scalar Structural Causal Model", fontsize=15, fontweight="bold")
    ax.axis("off")
    plt.tight_layout()
...
def print_selected_features(task):
    ...
    print("\nSELECTED FEATURES")
    print("=" * 100)
    print(f"{'feature':>8} {'node':>8} {'type':>15} {'observation':>28} {'K':>6} {'retention':>12} {'importance':>12}")
    ...
    for column, gid in enumerate(feature_ids):
        ...
        print(f"{'F' + str(column):>8} {int(gid):>8} {kind:>15} {observation_names[column]:>28} {k:>6} {float(retention[column]):>12.4f} {float(importance[column]):>12.4f}")
