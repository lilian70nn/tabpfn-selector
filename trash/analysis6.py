...
#     "ternary_op_probs": (3.0, 2.0, 2.0, 3.0),
...
def _impute_train_test(X_train, X_test):
    ...
    train_rows, train_cols = np.where(~np.isfinite(X_train))
    test_rows, test_cols = np.where(~np.isfinite(X_test))

    if len(train_rows) > 0:
        X_train[train_rows, train_cols] = means[train_cols]

    if len(test_rows) > 0:
        X_test[test_rows, test_cols] = means[test_cols]
...
def _local_label_consistency(X, y, k=10):
    ...
    raw_agreement = float(same_label.mean(axis=1).mean())
    ...
    normalized_consistency = np.nan if denominator <= 1e-12 else (raw_agreement - random_agreement) / denominator
...
def _observed_mlp_score(X_train, X_test, y_train, y_test, num_classes, epochs=500, seed=0):
...
def _evaluate_task(task, k=10, seed=0):
    ...
    observed_X_train = _to_numpy(task.X_train).astype(np.float32)
    observed_X_test = _to_numpy(task.X_test).astype(np.float32)
    ...
    observed_X_all = np.concatenate([observed_X_train, observed_X_test], axis=0)
    y_all = np.concatenate([y_train_np, y_test_np], axis=0)

    consistency = _local_label_consistency(observed_X_all, y_all, k=k)

    observed_mlp = _observed_mlp_score(
        observed_X_train,
        observed_X_test,
        y_train_np,
        y_test_np,
...
def run_analysis(prior, class_counts=(2, 3, 4), n_tasks=100, k=10, base_seed=0, output_csv="observed_consistency.csv"):
    rows = []
    ...
    print()
    print("=" * 60)
    print("CORRELATION WITH OBSERVED MLP")
    print("=" * 60)
    ...
    for num_classes in sorted(df["num_classes"].unique()):
        ...
        consistency_corr = subset[["observed_mlp", "normalized_local_consistency"]].corr().iloc[0, 1]
        r2_corr = subset[["observed_mlp", "target_latent_r2"]].corr().iloc[0, 1]

        print(f"{num_classes}-class | consistency vs MLP={consistency_corr:.4f} | target latent R2 vs MLP={r2_corr:.4f}")
    ...
    for num_classes in sorted(df["num_classes"].unique()):
        ...
        mean_mlp = subset["observed_mlp"].mean()
        mean_r2 = subset["target_latent_r2"].mean()
        median_r2 = subset["target_latent_r2"].median()

        print(
            f"{num_classes}-class | consistency mean={mean_consistency:.4f}, median={median_consistency:.4f} | "
            f"raw={raw_agreement:.4f}, random={random_agreement:.4f} | "
            f"target latent R2 mean={mean_r2:.4f}, median={median_r2:.4f} | observed MLP={mean_mlp:.4f}"
        )
...
if __name__ == "__main__":
    run_analysis(
        ...
        k=10,
        base_seed=0,
        output_csv="observed_consistency.csv",
    )